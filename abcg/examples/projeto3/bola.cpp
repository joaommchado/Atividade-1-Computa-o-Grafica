...
void Bola::initializeGL(GLuint program, std::string assetsPath, glm::vec3 scale, glm::vec3 translation, bool movementEnable) {

  m_program = program;
  m_movementEnable = movementEnable;
  ...
  bolaPosition = translation;

  loadModel(assetsPath);
}

glm::vec3 Bola::getBolaCoordenates(){

  return bolaPosition;
}

void Bola::moveX(float speedX){

  if(m_movementEnable)
    m_modelMatrix = glm::translate(m_modelMatrix, glm::vec3(speedX, 0.0f, 0.0f));
}

void Bola::moveZ(float speedZ){

  if(m_movementEnable)
    m_modelMatrix = glm::translate(m_modelMatrix, glm::vec3(0.0f, 0.0f, speedZ));
}

void Bola::paintGL(glm::mat4 viewMatrix, double deltaTime) {

  if(!m_movementEnable) //automove
    m_modelMatrix = glm::rotate(m_modelMatrix, glm::radians(-45.0f) * (float)deltaTime, glm::vec3(0, 0, 1));
